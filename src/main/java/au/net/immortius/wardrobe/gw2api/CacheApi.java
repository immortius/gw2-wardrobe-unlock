    ...
    public void run() throws IOException {
        ...
        for (CacheConfig apiCache : config.apiCaches) {
            if (!Strings.isNullOrEmpty(apiCache.getBaseUrl())) {
                cacher.cache(apiCache.getBaseUrl(), config.paths.getApiPath().resolve(apiCache.getCachePath()), apiCache.isBulkSupported(), apiCache.isStringIds(), apiCache.isGroupedSkins());
            }
        }
    }
}
